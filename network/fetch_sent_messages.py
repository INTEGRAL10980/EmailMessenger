...
def fetch_sent_messages(dir : Dir, email : ModifiedIMAP4_SSL, contact : Contact, account : Account) -> list[dict]:
    ...
    try:
        ...
        if email is False:
            return []
        if email.current_mailbox != email.sent_mailbox and email.sent_mailbox != None:
            result, data = email.select(email.sent_mailbox)
            if result != "OK": return []
        uid_list = get_uid_list(email, contact, account)
        ...
        if len(output) != 0:
            contact.LAST_SENT_UID = max(uid_list)
